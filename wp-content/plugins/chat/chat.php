						...
						<fieldset>
							...
							<table border="0" cellpadding="4" cellspacing="0">
								...
								<tr class="fff">
									...
									<td class="info"><?php _e( "Display theee user's avatar with the message?", $this->translation_domain ); ?></td>
								</tr>
						...
						foreach ( $rows as $row ) {
							...
							if ( $_POST['avatar'] == 'enabled' ) {
								...
								$name_color = wc_sanitize_hex_color( $_POST['name_color'] );
                                                        ...
                                                        $uname=get_user_by('login', stripslashes( html_entity_decode( $row->name ) ));
                                                        $prepend .= ' <span class="name" style="background: ' . $name_color . ';">' .$uname->display_name. '</span>';

                                                        
                                                        $append="";
							if ( $_POST['date_show'] == 'enabled' ) {
								$append .= ' <span class="date" style="background: ' . wc_sanitize_hex_color( $_POST['date_color'] ) . ';">' . date_i18n( get_option( 'date_format' ), strtotime( $row->timestamp ) + get_option( 'gmt_offset' ) * 3600, false ) . '</span>';
							}
							if ( $_POST['time_show'] == 'enabled' ) {
								$append .= ' <span class="time" style="background: ' . wc_sanitize_hex_color( $_POST['date_color'] ) . ';">' . date_i18n( get_option( 'time_format' ), strtotime( $row->timestamp ) + get_option( 'gmt_offset' ) * 3600, false ) . '</span>';
							}
							...
							$text[ $row->id ] = " <div id='row-" . strtotime( $row->timestamp ) . "' class='row'>
                                                                <div class='col-xs-2 user-details' data-name='".stripslashes( html_entity_decode( $row->name ) )."'>";
                                                                if(in_array('mentor', $uname->roles)){$text[ $row->id ].="<a href='".home_url().'/instructor/'.stripslashes( html_entity_decode( $row->name ) )."'>";}
                                                                    $text[ $row->id ].=$prepend;
                                                                 if(in_array('mentor', $uname->roles)){$text[ $row->id ].="</a>";}
                                                                $text[ $row->id ].="</div>
                                                                <div class='col-xs-8 user-message'>
                                                                    <span class='message' style='color: " . wc_sanitize_hex_color( $_POST['text_color'] ) . "'>" . convert_smilies( $message ) . "</span>
                                                                </div>
                                                                <div class='col-xs-2 user-date'>{$append}</div>
                                                                <div class='chat-clear'></div></div>";
