    ...
    <section id="course-summary">
        ...
        <div class="entry-content-excerpt <?php echo ($course_media == '' ? 'entry-content-excerpt-right' : '' ); ?>">
			...
			<?php echo do_shortcode( '[course_join_button]' ); ?>
                <?php else :?>
                    <?php $slug=$course->details->post_name;?>
                        <button class="apply-button apply-button-enrolled " data-link="<?php echo home_url()."/courses/$slug/units/";?>"><?php _e('Lessons', 'study');?></button>
                <?php endif; ?>
            </div>
        </div>
    </section>
